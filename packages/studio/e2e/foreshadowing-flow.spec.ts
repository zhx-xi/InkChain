...
test.describe("Foreshadowing — 核心创作流E2E", () => {
  ...
  // ── PR #443: lastMentionedChapter + null payoff notes ──

  test("11. AI提取结果显示最近提及章节和预期回收", async ({ page }) => {
    // Mock extraction with lastMentionedChapter and both null/non-null payoff
    await mockAiExtract(page, [
      {
        id: "fs-e2e-pr443-1",
        title: "有回收章节的伏笔",
        description: "这条有明确的预期回收章节",
        type: "情节伏笔",
      ...
      },
      {
        id: "fs-e2e-pr443-2",
        title: "未指定回收章节的伏笔",
        description: "这条的expectedPayoffChapter为null",
        type: "设定伏笔",
    ...
    // Verify lastMentionedChapter displays
    await expect(page.getByText("最近提及：第 8 章")).toBeVisible({ timeout: 8_000 });
    await expect(page.getByText("最近提及：第 6 章")).toBeVisible({ timeout: 3_000 });

    // Verify expectedPayoffChapter with value
    await expect(page.getByText("预期回收：第 15 章")).toBeVisible({ timeout: 3_000 });

    // Verify null expectedPayoffChapter shows "大纲未指定"
    await expect(page.getByText("大纲未指定")).toBeVisible({ timeout: 3_000 });
  });

  test("12. 已保存伏笔显示大纲未指定标记", async ({ page }) => {
    // Mock AI extract with entires that get applied
    await mockAiExtract(page, [
      {
        id: "fs-e2e-pr443-3",
        title: "无回收章节的伏笔",
        description: "保存后应显示大纲未指定",
        type: "角色伏笔",
        expectedPayoffChapter: null,
    ...
    await page.getByRole("button", { name: "开始提取" }).click();
    await expect(page.getByText("无回收章节的伏笔")).toBeVisible({ timeout: 8_000 });

    // Apply to save to main list
    await page.getByRole("button", { name: "应用所有" }).click();
    await expect(page.getByText("AI 提取伏笔")).not.toBeVisible({ timeout: 5_000 });

    // In the main list, the entry should show "大纲未指定"
    await expect(page.getByText("大纲未指定")).toBeVisible({ timeout: 5_000 });
    await expect(page.getByText("无回收章节的伏笔")).toBeVisible({ timeout: 3_000 });
  });
