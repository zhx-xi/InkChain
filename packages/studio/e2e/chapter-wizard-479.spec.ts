...
test.describe("ChapterWizard — 章节创作向导 E2E", () => {
  test.beforeAll(async () => {
    await seedSidebarNav();
  });

  test("1. Normal: 页面加载显示向导内容", async ({ page }) => {
    // Given 一个已存在的书籍
    // When 导航到章节创作向导
  ...
  test("2. Error: 无效书籍 ID 不崩溃", async ({ page }) => {
    // Given 一个不存在的书籍 ID
    const invalidId = "non-existent-wizard-book-99999";
  ...
  test("3. Edge: 加载完毕后的页面内容检测", async ({ page }) => {
    // Given 一个已存在的书籍
    await page.goto(`/#/chapter-wizard/${BOOK_ID}`);
    await page.waitForLoadState("networkidle", { timeout: 15_000 }).catch(() => {});
