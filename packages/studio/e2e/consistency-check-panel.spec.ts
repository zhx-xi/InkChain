...
test.describe("ConsistencyCheckPanel — 一致性检查 E2E", () => {
  test.beforeAll(async () => {
    await seedSidebarNav();
  });

  test("1. Normal: 页面加载并显示一致性检查界面", async ({ page }) => {
    // Given 一个已存在书籍
    // When 导航到一致性检查页面
  ...
  test("3. Edge: 不同页面间导航后再返回", async ({ page }) => {
    // Given 一个已存在的书籍
    // When 先访问书籍设置页再导航到一致性检查页
  ...
  test("4. Empty: 无章节书籍的一致性检查页加载", async ({ page }) => {
    // Given 一个存在但无章节的特殊书籍
    const emptyBookId = "e2e-consistency-empty";
    try {
      await page.request.post("/api/books", {
        data: { id: emptyBookId, title: "E2E Consistency Empty Book", genre: "xianxia", status: "active" },
      });
    } catch {
      // 如果 API 不可用，跳过
    }
    ...
    // When 导航到空书籍的一致性检查页
    await page.goto(`/#/consistency/${emptyBookId}`);
    await page.waitForTimeout(5_000);
