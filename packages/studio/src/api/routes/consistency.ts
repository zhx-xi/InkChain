...
import { checkConsistency } from "@inkchain/inkchain-core";
import { loadWorld } from "@inkchain/inkchain-core";
...
export function createConsistencyRouter(root: string) {
  ...
  router.post("/check", async (c) => {
    ...
    if (worldId) {
      ...
      const { WorldConfigSchema } = await import("@inkchain/inkchain-core");
