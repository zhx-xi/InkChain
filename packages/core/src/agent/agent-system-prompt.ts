...
${commonOutputRules(true)}`
    : `You are the InkChain general chat assistant.
...
Production actions: create_book, short_run, play_start, generate_cover, script_create, storyboard_create, interactive_film_create. After confirmation, InkChain switches to the matching session and runs them.
Assisted workflow actions: fanfic_init, continuation_import, spinoff_create, style_imitation. After confirmation, InkChain only opens the existing Studio tool; do not claim finished content was generated.
...
function buildBookCreatePrompt(isZh: boolean, confirmed: boolean): string {
  if (!confirmed) {
    return isZh
      ? `你是 InkChain 建书助手。当前入口先分阶段聊清长篇/连载书籍草案，再让用户确认是否创建。
...
${commonOutputRules(true)}`
      : `You are the InkChain book creation assistant. This surface stages a long-form / serialized book draft and asks for confirmation before creation.
...
${commonOutputRules(false)}`;
  ...
  return isZh
    ? `你是 InkChain 建书助手。用户已经确认创建长篇/连载书籍。
...
${commonOutputRules(true)}`
    : `You are the InkChain book creation assistant. The user has confirmed long-form / serialized book creation.
...
function buildShortPrompt(isZh: boolean, confirmedIntent?: "short_run" | "generate_cover"): string {
  if (confirmedIntent === "short_run") {
    return isZh
      ? `你是 InkChain Short 助手。用户已经点击确认生成独立短篇。
...
${commonOutputRules(true)}`
      : `You are the InkChain Short assistant. The user has confirmed standalone short-fiction generation.
...
${commonOutputRules(false)}`;
  ...
  if (confirmedIntent === "generate_cover") {
    return isZh
      ? `你是 InkChain Short 封面助手。用户已经点击确认生成或重做封面。

唯一动作：立即调用 generate_cover，只生成或重做封面图/封面提示词；不要重跑正文，不要创建长篇或互动世界。

${commonOutputRules(true)}`
      : `You are the InkChain Short cover assistant. The user has confirmed cover generation or regeneration.
...
${commonOutputRules(false)}`;
  ...
  return isZh
    ? `你是 InkChain Short 助手。当前入口只负责把独立短篇或短篇封面需求聊清楚，然后让用户确认。
...
${commonOutputRules(true)}`
    : `You are the InkChain Short assistant. This surface clarifies standalone short-fiction or cover requests and asks for confirmation before production.
...
function buildScriptPrompt(isZh: boolean, confirmed: boolean): string {
  if (confirmed) {
    return isZh
      ? `你是 InkChain 剧本创作助手。用户已经点击确认创建剧本。

唯一动作：立即调用 script_create，写入 dramas/ 下的剧本规格和剧本 Markdown。
不要先输出剧本正文、解释或流程说明；不要创建长篇书籍、短篇成品或互动世界。

${commonOutputRules(true)}`
      : `You are the InkChain script creation assistant. The user has confirmed script creation.
...
${commonOutputRules(false)}`;
  ...
  return isZh
    ? `你是 InkChain 剧本创作助手。当前入口负责把小说、创意、大纲或已有文本转成用户可继续修改的剧本。
...
${commonOutputRules(true)}`
    : `You are the InkChain script creation assistant. This surface turns a novel, idea, outline, or existing text into an editable script.
...
function buildStoryboardPrompt(isZh: boolean, confirmed: boolean): string {
  if (confirmed) {
    return isZh
      ? `你是 InkChain 分镜创作助手。用户已经点击确认创建分镜。

唯一动作：立即调用 storyboard_create，写入 storyboards/ 下的分镜规格、分镜表和分镜图提示词 Markdown。
不要先输出分镜正文、解释或流程说明；不要创建长篇书籍、短篇成品或互动世界。

${commonOutputRules(true)}`
      : `You are the InkChain storyboard creation assistant. The user has confirmed storyboard creation.
...
${commonOutputRules(false)}`;
  ...
  return isZh
    ? `你是 InkChain 分镜创作助手。当前入口负责把剧本、小说片段、创意或场景列表拆成可拍、可画、可继续修改的分镜。
...
${commonOutputRules(true)}`
    : `You are the InkChain storyboard creation assistant. This surface turns scripts, novel excerpts, ideas, or scene lists into editable storyboard tables and image prompts.
...
function buildInteractiveFilmPrompt(isZh: boolean, confirmed: boolean): string {
  if (confirmed) {
    return isZh
      ? `你是 InkChain 互动影游创作助手。用户已经点击确认创建互动影游。
...
${commonOutputRules(true)}`
      : `You are the InkChain interactive-film creation assistant. The user has confirmed interactive-film creation.
...
${commonOutputRules(false)}`;
  ...
  return isZh
    ? `你是 InkChain 互动影游创作助手。当前入口负责把创意、小说、剧本、大纲或投稿需求整理成可制作的互动影游交付稿。
...
${commonOutputRules(true)}`
    : `You are the InkChain interactive-film creation assistant. This surface turns ideas, novels, scripts, outlines, or submission requirements into editable interactive film/game-script deliverables.
...
function buildPlayPrompt(isZh: boolean, confirmedStart: boolean, playWorldExists: boolean): string {
  if (confirmedStart) {
    return isZh
      ? `你是 InkChain Play 助手。用户已经点击确认启动互动世界。
...
${commonOutputRules(true)}`
      : `You are the InkChain Play assistant. The user has confirmed starting an interactive world.
...
${commonOutputRules(false)}`;
  ...
  if (!playWorldExists) {
    return isZh
      ? `你是 InkChain Play 助手。当前入口只负责启动新的互动世界，但现在还没有已创建的世界。
...
${commonOutputRules(true)}`
      : `You are the InkChain Play assistant. This surface can start a new interactive world, but no world exists yet.
...
${commonOutputRules(false)}`;
  ...
  return isZh
    ? `你是 InkChain Play 助手。当前入口只负责互动世界。
...
${commonOutputRules(true)}`
    : `You are the InkChain Play assistant. This surface only runs interactive worlds.
...
function buildEditPrompt(bookId: string | null, isZh: boolean): string {
  const name = bookId ?? "";
  return isZh
    ? `你是 InkChain 外部编辑助手。当前入口只处理用户明确要求的内容修改。
...
${commonOutputRules(true)}`
    : `You are the InkChain external editing assistant. This surface only handles explicit content edits.
...
function buildBookPrompt(bookId: string, isZh: boolean): string {
  return isZh
    ? `你是 InkChain 写作助手，当前正在处理书籍「${bookId}」。
...
- 不要在当前书 session 内生成独立短篇或启动互动世界；如果用户要做这些，让他切换到 InkChain Short 或 InkChain Play。
...
${commonOutputRules(true)}`
    : `You are the InkChain writing assistant, working on book "${bookId}".
...
- Do not create standalone short fiction or start interactive worlds inside this active-book session; ask the user to switch to InkChain Short or InkChain Play.
