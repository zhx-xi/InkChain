...
InkOS v3.0 — 最终交付版本。完整实现六大创作扩展（会话归档、角色分层、关系图、时间线、分卷、世界观场景）、AI 辅助增强（AI 关系提取、角色生成对比）、安全加密存储和开放世界 AI 生成能力。

### Major Features

- **API Key 加密存储**（#48）: AES-256-GCM 加密 API Key，透明集成到 secrets 存储，自动迁移旧明文 Key
- **关系图 AI 标注**（#99）: 基于角色卡和正文分析自动标注角色关系，置信度过滤 + 手动确认
- **AI 卷纲规划**（#97）: 自动检测卷弧覆盖，生成逐卷章节规划，支持自定义配置
- **编辑仪表盘**（#98）: 集成进度、角色、关系、时间线、世界设定的全局创作视图
- **AI 导入提取 MVP**（#89）: 从已有文本提取结构化世界观（实体/关系/规则），导入并审查
- **跨平台发布正式版**（#100）: 起点/番茄双平台适配器，内容校验 + 格式预览 + 一键发布
- **Persona A/B 对比测试**（#95）: 角色版本快照、结构化 diff、A/B 效果对比
- **Skill/Persona UI 编辑**（#96, #94）: Studio 内可视化编辑 Skill 和 Persona 配置
- **开放世界 AI 生成**（#102）: AI 增强的世界生成（章节/角色/事件三模式），用户确认后可导入
- **AI 续写增强**（#103）: World/Relation/Timeline/Foreshadowing 完整上下文集成，多候选续写 + 冲突检测
- **全量回归测试**（#105）: v1.5b → v3.0 全线功能回归，性能基准验证

### Studio UX

- 侧边栏分卷拖拽排序（#42-#43）
- 时间线可视化：事件图表、角色过滤、时间线滑动
- 全文搜索（Ar-2）: 跨章节/会话智能搜索
- 写作面板优化（R-9）: 续写入口、快速操作栏
- 统计面板：Token 用量、字数统计、章节排名
- 成就系统：7 里程碑进度追踪
- 命令面板：Cmd+K 快速操作
- 导出增强：EPUB 真实导出 + HTML 预览

### Data & Security

- **AES-256-GCM 加密存储**（#48）：API Key 全量加密，透明集成
- **分卷管理**：卷级 CRUD、侧边栏拖拽排序
- **角色分层**：major/minor 角色目录，分页浏览
- **关系图**：可视化关系图谱、AI 自动标注
- **时间线**：事件 CRUD、重要性标记、角色过滤
- **世界观场景**：AI 提取 + AI 生成 + 确认导入

### Bug Fixes

- 修复服务器启动崩溃（PR #186）
- 修复 v2.5 集成联调发现的 TS build 错误
- 修复多个 UI 交互状态更新问题
