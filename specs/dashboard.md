...
| Schema | 类型 | 定义 |
|--------|------|------|
| `LLMConfigSchema` | object | {
  provider: z.enum(["anthropic", "openai", "custom"] |
| `NotifyChannelSchema` | discriminatedUnion | "type", [
  z.object({
    type: z.literal("telegram" |
| `DetectionConfigSchema` | object | {
  provider: z.enum(["gptzero", "originality", "custom"] |
| `QualityGatesSchema` | object | {
  maxAuditRetries: z.number( |
| `FoundationConfigSchema` | object | {
  reviewRetries: z.number( |
| `WritingConfigSchema` | object | {
  reviewRetries: z.number( |
| `AgentLLMOverrideSchema` | object | {
  model: z.string( |
